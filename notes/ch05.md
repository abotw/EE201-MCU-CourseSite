...
- **Bitwise Operators (`&`, `|`, `~`, `^`)**: Operate directly on the individual binary bits of a variable 3.
    ...
    - **Bitwise Shift Operators (`<<`, `>>`)**: **Left Shift (`<<`)** is equivalent to multiplying by $2^n$ (e.g., `x << 2` is $x * 4$)66. **Right Shift (`>>`)** is equivalent to dividing by $2^n$77. These are highly efficient.
